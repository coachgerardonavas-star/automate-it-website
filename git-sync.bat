...
REM
REM  Se usa la forma "<commit>:" para pedir el arbol raiz, NO "<commit>^{tree}".
REM  El acento circunflejo es el caracter de escape de cmd.exe y no sobrevive
REM  intacto dentro de los backticks de un for /f: la primera version de esto
REM  devolvia vacio, la comparacion nunca daba igual, y el script creaba una
REM  instantanea identica cada noche. "<commit>:" da exactamente el mismo sha
REM  sin necesitar escape.
set "BASETREE="
if defined PREV for /f "usebackq tokens=*" %%x in (`git rev-parse %PREV%: 2^>nul`) do set "BASETREE=%%x"
if not defined PREV for /f "usebackq tokens=*" %%x in (`git rev-parse HEAD: 2^>nul`) do set "BASETREE=%%x"
