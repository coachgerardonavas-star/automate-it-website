...
function successAndCancelUrls(lang: "es" | "en"): { success_url: string; cancel_url: string } {
  if (lang === "en") {
    return {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Browser calls must originate from an approved Automate IT surface.
    // Origin-less server-to-server calls remain possible, but are still rate-limited.
    ...
    if (env.RATE_LIMITER && ip && !(await env.RATE_LIMITER.limit({ key: ip })).success) {
      return jsonResponse({ error: "Too many attempts. Try again shortly." }, 429, cors);
    }
    ...
    if (!stripeRes.ok || !data?.url) {
      // Keep diagnostics useful without copying the complete upstream response headers/body into logs.
      console.error("STRIPE_ERROR", JSON.stringify({
        status: stripeRes.status,
        contentType,
        errorType: data?.error?.type ?? null,
        errorCode: data?.error?.code ?? null,
        errorParam: data?.error?.param ?? null,
      }));
